...
//para crear una publicación
    static async create(req: Request, res: Response) {
        try {
            ...
            const { description, creatorUserId, publicationType, petName, petType, petBreed, petColor, petSize, petAge, petURL, latitude, longitude, reference_address } = req.body;
            ...
            const newLocation = await Location.create(latitude, longitude, creatorUserId, reference_address);
            ...
            await Publication.save(newPublication);
    ...
    static async disable(req: Request, res: Response) {
        try {
            ...
            await Publication.disable(id);
            return res.status(200).json({ message: 'Publicación desactivada con éxito' });
        } catch (error: any) {
            console.error(error);
            res.status(500).json({ message: 'Error al desactivar la publicación', error: error.message });
        }
    }
    ...
    static async update(req: Request, res: Response) {
        try {
            ...
            //actualiza
            const updatedPublication = await Publication.update(id, dataToUpdate);

            return res.status(200).json({ message: 'Publicación actualizada con éxito', data: updatedPublication });
        } catch (error: any) {
            console.error(error);
